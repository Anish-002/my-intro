      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-24 gap-8">
          <div className="max-w-3xl">
            ...
            <h2 className="text-5xl md:text-7xl font-bold mb-8 text-slate-900">Selected <span className="text-emerald-500">Creations</span></h2>
            <p className="text-xl text-slate-500 leading-relaxed">
              Discover a gallery of engineering excellence, where design meets advanced functionality.
            </p>
          </div>
          <button className="px-8 py-4 bg-white border border-slate-200 shadow-sm rounded-full font-bold text-sm flex items-center gap-2 hover:bg-slate-50 transition-all">
            Explore All Work
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {projects.map((project, idx) => (
            <motion.div
              ...
              className={`group bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full hover:shadow-2xl transition-all duration-500 ${
            ...
            >
              <div className="relative aspect-video overflow-hidden">
                ...
                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center gap-4 backdrop-blur-sm">
                  <a href={project.live} target="_blank" className="p-4 bg-primary text-white rounded-full hover:scale-110 transition-transform shadow-lg">
                    <Play className="w-6 h-6 fill-current" />
                  </a>
                  <a href={project.github} target="_blank" className="p-4 bg-white text-slate-900 rounded-full hover:scale-110 transition-transform shadow-lg">
                    <Github className="w-6 h-6" />
                  </a>
                </div>
                {project.featured && (
                  <div className="absolute top-6 left-6 bg-primary text-white px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 shadow-lg">
              ...
              <div className="p-10 flex-1 flex flex-col">
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] font-bold uppercase tracking-widest text-primary px-3 py-1 rounded-full bg-primary/5 border border-primary/10">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="pt-8 border-t border-slate-50 flex items-center justify-between">
                  <a href={project.live} target="_blank" className="text-slate-900 font-bold text-sm flex items-center gap-2 group/link hover:text-primary transition-colors">
                    View Case Study
