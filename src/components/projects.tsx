              ...
              <div className="p-12 flex-1 flex flex-col">
                <div className="flex flex-wrap gap-3 mb-8">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-400 px-4 py-1.5 rounded-xl bg-slate-50 border border-slate-100">
                      {tag}
                    </span>
                  ))}
                </div>
                <h3 className="text-3xl font-black mb-4 text-slate-900">{project.title}</h3>
                <p className="text-lg text-slate-500 leading-relaxed mb-10 flex-1 font-medium">
                  {project.description}
                </p>
                <div className="pt-8 border-t border-slate-50 flex items-center justify-between">
                  <a href={project.live} target="_blank" className="text-slate-900 font-black text-sm font-mono uppercase tracking-widest flex items-center gap-3 hover:text-blue-600 transition-colors">
                    View Live Project
                    <ChevronRight className="w-5 h-5" />
                  </a>
                </div>
