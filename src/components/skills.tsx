    ...
    <section id="skills" className="py-32 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 xl:px-12 max-w-[1400px]">
        <div className="max-w-6xl mb-24">
          ...
          <h2 className="text-5xl md:text-7xl xl:text-8xl font-black mb-8 text-slate-900">Technical <span className="text-gradient">Arsenal</span></h2>
          <p className="text-xl xl:text-2xl text-slate-500 font-medium max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 xl:gap-12">
          {skillCategories.map((cat, idx) => (
            <motion.div
              ...
              className={`group p-10 xl:p-14 glass-card rounded-[3rem] hover:shadow-2xl transition-all duration-500 hover:-translate-y-2`}
            >
              <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-white bg-gradient-to-br ${cat.color} mb-10 shadow-lg shadow-${cat.color.split(' ')[0].split('-')[1]}-500/20 group-hover:scale-110 transition-transform duration-500`}>
                <div className="scale-125">{cat.icon}</div>
              </div>
              <h3 className="text-3xl font-bold mb-6 text-slate-900">{cat.title}</h3>
              <p className="text-base md:text-lg text-slate-500 mb-10 font-medium leading-relaxed">{cat.description}</p>
              
              <div className="flex flex-wrap gap-3">
                {cat.skills.map((skill, i) => (
                  <span key={i} className="px-4 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] md:text-xs font-mono font-bold text-slate-600 group-hover:bg-white group-hover:border-blue-100 transition-colors">
                    {skill}
                  </span>
                ))}
              </div>
